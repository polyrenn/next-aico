...
const CreateBranch:FC<ModalProps> = (props) => {
    ...
    const createBranch = async (values: {name: string, address: string, company: string}, actions:any) => {
      ...
      const data = {
        name: name,
        branchId: Math.floor(100000 + Math.random() * 900000),
        address: address
      }


      const res = await fetch(`/api/Common/CreateBranch?id=${company}`, {
        method: 'post',
